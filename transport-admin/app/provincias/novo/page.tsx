...
export default function NovaProvincia() {
  ...
  useEffect(() => {
    ...
    // Add center point marker (circle instead of pin)
    ...
    const fetchMunicipios = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const unassigned = (result.data || []).filter((m: any) => !m.provinciaId);
          setMunicipios(unassigned);
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.codigo.length < 2 || formData.codigo.length > 3) {
      showNotification('Código deve ter 2-3 letras', 'error');
      return;
    }
          ...
          await Promise.all(
            selectedMunicipios.map(municipioId =>
              fetch(`/api/municipios/${municipioId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ provinciaId: provincia.id }),
              })
            )
          );
        }

        showNotification('Província criada com sucesso!', 'success');
        setTimeout(() => router.push(`/provincias/${provincia.id}`), 1500);
      } else {
        const error = await response.json();
        showNotification(error.error || 'Erro ao criar província', 'error');
      }
    } catch (error) {
      showNotification('Erro ao criar província', 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex items-center space-x-4 mb-6">
          ...
          <div>
            <h2 className="text-3xl font-bold text-gray-900">Nova Província</h2>
            <p className="text-gray-600 mt-1">Crie uma nova província no sistema</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
            ...
            <p className="text-xs text-gray-500">
              💡 Dica: Clique no mapa para definir o centro da província
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-bold text-gray-900 mb-4">
              Associar Municípios ({selectedMunicipios.length} selecionados)
            </h3>
            ...
            {loadingMunicipios ? (
              ...
              <div className="text-sm text-gray-600 p-4 bg-gray-50 rounded-lg border border-gray-200">
                Nenhum município disponível para associar. Todos os municípios já estão associados a outras províncias.
              </div>
            ) : (
                ...
                {municipios.map((municipio) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-900">{municipio.nome}</p>
                      <p className="text-xs text-gray-500">{municipio.codigo}</p>
                    </div>
                  </label>
                ))}
              </div>
