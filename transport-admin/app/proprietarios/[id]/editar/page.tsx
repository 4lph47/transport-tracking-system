...
export default function EditarProprietario() {
  ...
  async function fetchProprietario(id: string) {
    try {
      ...
      setFormData({
        ...
        tipoProprietario: data.tipoProprietario || "Empresa",
      });
      ...
      setTimeout(() => router.push('/proprietarios'), 2000);
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleCertificadoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        showNotification('Certificado carregado com sucesso', 'success');
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.nome.trim()) {
      showNotification('Por favor, insira o nome', 'error');
      return;
    }
    ...
    try {
      const response = await fetch(`/api/proprietarios/${params.id}`, {
        ...
        body: JSON.stringify({
          ...
          tipoProprietario: formData.tipoProprietario,
      ...
      if (response.ok) {
        showNotification('Proprietário actualizado com sucesso!', 'success');
        setTimeout(() => {
          router.push(`/proprietarios/${params.id}`);
        }, 1500);
      } else {
        const error = await response.json();
        showNotification(error.error || 'Erro ao actualizar proprietário', 'error');
      }
    } catch (error) {
      console.error('Erro ao actualizar proprietário:', error);
      showNotification('Erro ao actualizar proprietário', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const isEmpresa = formData.tipoProprietario?.toLowerCase() === 'empresa';
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="max-w-[1200px] mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center space-x-4">
          <button
            onClick={() => router.push(`/proprietarios/${params.id}`)}
          ...
          <div>
            ...
            <p className="text-gray-600 mt-1">Actualizar informações do proprietário</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl border border-gray-200 p-6">
                <h3 className="text-lg font-bold text-gray-900 mb-4">
                  {isEmpresa ? 'Logo da Empresa' : 'Foto'}
                </h3>
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-6">
                <h3 className="text-lg font-bold text-gray-900 mb-4">
                  {isEmpresa ? 'Informações da Empresa' : 'Informações Pessoais'}
                </h3>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    <label className="block text-sm font-semibold text-gray-900 mb-2">
                      Nome {isEmpresa ? 'da Empresa' : 'Completo'} *
                    </label>
                    <input
                      type="text"
                      value={formData.nome}
                      ...
                      placeholder={isEmpresa ? "Ex: Transportes Rápidos Lda" : "Ex: João Silva"}
                      required
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900 text-gray-900"
                      placeholder={isEmpresa ? "Ex: 100000000006F" : "Ex: 110000000000A"}
                      required
                    />
                  </div>
