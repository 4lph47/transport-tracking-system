...
export default function EditarMotorista() {
  ...
  const [currentTransporteId, setCurrentTransporteId] = useState<string | null>(null);
  ...
  async function fetchMotorista(id: string) {
    try {
      ...
      setFormData({
        nome: data.nome || "",
        bi: data.bi || "",
        cartaConducao: data.cartaConducao || "",
        telefone: data.telefone?.replace('+258', '') || "",
        email: data.email || "",
        ...
        numeroEmergencia: data.numeroEmergencia?.replace('+258', '') || "",
        contatoEmergencia: data.contatoEmergencia || "",
        deficiencia: data.deficiencia || "",
        dataEmissaoBI: data.dataEmissaoBI ? new Date(data.dataEmissaoBI).toISOString().split('T')[0] : "",
        dataValidadeBI: data.dataValidadeBI ? new Date(data.dataValidadeBI).toISOString().split('T')[0] : "",
        dataEmissaoCarta: data.dataEmissaoCarta ? new Date(data.dataEmissaoCarta).toISOString().split('T')[0] : "",
      ...
      // Load current transport if exists
      if (data.transporte) {
        setSelectedTransporte(data.transporte);
        setCurrentTransporteId(data.transporte.id);
      }
    } catch (error) {
  ...
  async function fetchTransportes() {
    ...
    try {
      const url = currentTransporteId 
        ? `/api/transportes?availableOnly=true&includeTransportId=${currentTransporteId}&limit=1000`
        : '/api/transportes?availableOnly=true&limit=1000';
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showNotification(error.error || 'Erro ao atualizar motorista', 'error');
      }
    } catch (error) {
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="max-w-[1400px] mx-auto p-6 space-y-6">
        <div className="flex items-center space-x-4">
          <button
            onClick={() => router.push(`/motoristas/${params.id}`)}
          ...
          <div>
            <h2 className="text-3xl font-bold text-gray-900">Editar Motorista</h2>
            <p className="text-gray-600 mt-1">Atualize as informações do motorista</p>
          </div>
        </div>
